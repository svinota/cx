...
class Server(object):
    ...
    def rpc(self, fd):
        ...
        if hasattr(self, name):
            ...
            try:
                ...
                print >>sys.stderr, "servererror: ", traceback.print_exc()
                self._err(fd, tag, e.args[0])
                return -1
            except Error, e:
                print >>sys.stderr, "error: ", traceback.print_exc()
                self._err(fd, tag, e.args[0])
                return -1
            except Exception, e:
    ...
    def _srvTauth(self, type, tag, vals):
        ...
        obj = File('#a', py9ppki.AuthFs(uname))
        obj.user = uname
...
class Client(object):
    ...
    def login(self, user, passwd, authsrv):
        ...
        if afid != nofid:
            py9ppki.clientAuth(self, afid, user)
